...
export function ReturnStatusBadge({ status, className = '' }: ReturnStatusBadgeProps) {
  switch (status) {
    ...
    default:
      return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 ${className}`}>
          {status}
        </span>
      );
  }
